...
export = {
  ...
  async runInteraction(client: any, interaction: any) {
    ...
    if (user) {
      let i = 0;
      const filteredTargetMessages: any[] = [];
      (await messagesToDelete).filter((msg: any) => {
      ...
      await interaction.channel.bulkDelete(filteredTargetMessages, true).then((messages: any) => {
        const embedTarget = {
          color: 0x735b8b,
          description: `${messages.size} messages de ${user} supprimé`,
        };
        interaction.reply({embeds: [embedTarget]}).then(setTimeout(() => interaction.deleteReply(), 10000));
      });
      // Sinon, on supprime les X derniers messages dans le salon
    } else {
      await interaction.channel.bulkDelete(amountToDelete, true).then((messages: any) => {
        const embedNoTarget = {
          color: 0x735b8b,
          description: `${messages.size} messages supprimé`,
        };
        interaction.reply({embeds: [embedNoTarget]}).then(setTimeout(() => interaction.deleteReply(), 2000));
      });
    }
  },
};
